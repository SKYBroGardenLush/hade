...
type IResponse interface {
	...
	// Cookie
	ISetCookie(key string, val string, maxAge int, path, domian string, secure, httpOnly bool) IResponse
...
//func (ctx *Context) ISetHeader(key string, val string) {
...
func (ctx *Context) IJsonp(obj interface{}) IResponse {
	...
	_, err = ctx.Writer.Write([]byte(ret))
	if err != nil {
		return ctx
	}
...
func (ctx *Context) IText(format string, values ...interface{}) IResponse {
	byt := []byte(fmt.Sprintf(format, values...))
...
func (ctx *Context) ISetCookie(key string, val string, maxAge int, path, domian string, secure, httpOnly bool) IResponse {
	if path == "" {
		path = "/"
	}
	http.SetCookie(ctx.Writer, &http.Cookie{
		Name:     key,
		Value:    url.QueryEscape(val), //过滤防止xss??
		MaxAge:   maxAge,
		Path:     path,
		Domain:   domian,
		SameSite: 1,
