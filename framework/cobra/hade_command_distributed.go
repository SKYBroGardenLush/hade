...
//serviceName 这个服务唯一的名字，不允许带空格
//spec 具体的执行时间
//cmd 具体执行命令
//holdTime 表示如果我选择上了，这次选择的时间，也就是锁释放的时间
func (c *Command) AddDistributedCronCommand(serviceName string, spec string, cmd *Command, holdTime time.Duration) {
	root := c.Root()
	//初始化cron
	if root.Cron == nil {
		root.Cron = cron.New(cron.WithParser(cron.NewParser(cron.SecondOptional | cron.Minute | cron.Hour | cron.Dom | cron.Minute | cron.Dow | cron.Descriptor)))
	...
	//复制要执行的command 为cronCmd, 并设置为rootCmd
	var cronCmd Command
	ctx := root.Context()
	cronCmd = *cmd
	cronCmd.args = []string{}
	...
	root.Cron.AddFunc(spec, func() {
		...
		fmt.Println(selectedAppID, appID)
