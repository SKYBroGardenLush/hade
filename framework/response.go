...
type IResponse interface {
	...
	// Cookie
	SetCookie(key string, val string, maxAge int, path, domian string, secure, httpOnly bool) IResponse
...
func (ctx *Context) Jsonp(obj interface{}) IResponse {
	...
	_, err = ctx.responseWriter.Write([]byte(ret))
	if err != nil {
		return ctx
	}
...
func (ctx *Context) SetCookie(key string, val string, maxAge int, path, domian string, secure, httpOnly bool) IResponse {
	if path == "" {
		path = "/"
	}
