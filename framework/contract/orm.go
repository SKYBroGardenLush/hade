...
type DBConfig struct {
	//以下配置关于dsn
	WriteTimeout string `yaml:"write_timeout"'` //写超出时间
	Loc          string `yaml:"loc"`            //时区
	Port         int    `yaml:"port"`           //端口
	ReadTimeout  string `yaml:"read_timeout"`   //读超时间
	Charset      string `yaml:"charset"`        //字符集
	ParseTime    bool   `yaml:"parse_time"`     //是否解析时间
	Protocol     string `yaml:"protocol"`       //传输协议
	Dsn          string `yaml:"dsn"`            //直接传递dsn,如果传递了，关于其他dsn的配置均无效
	...
	//以下配置关于gorm
	*gorm.Config //集成gorm配置
}

func (cfg *DBConfig) FormatDsn() (string, error) {
